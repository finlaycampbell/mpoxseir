...
public:
  ...
  struct shared_state {
    ...
    struct dim_type {
      ...
      dust2::array::dimensions<2> new_target_met_adults_t;
  ...
  struct internal_state {
    ...
    std::vector<real_type> new_target_met_adults_t;
  ...
  static shared_state build_shared(cpp11::list parameters) {
    ...
    dim.new_target_met_adults_t.set({static_cast<size_t>(n_group), static_cast<size_t>(n_vax)});
  ...
  static internal_state build_internal(const shared_state& shared) {
    ...
    std::vector<real_type> new_target_met_adults_t(shared.dim.new_target_met_adults_t.size);
    std::vector<real_type> seed(shared.dim.seed.size);
    return internal_state{n_IrR, n_IdD, coverage_target_1st_dose_children, coverage_target_1st_dose_adults, coverage_target_2nd_dose_adults, max_vax_remaining, I_infectious, delta_R, delta_D, daily_doses_children_t, daily_doses_adults_t, target_met_children_t, target_met_adults_t, new_D, prop_infectious, lambda_hc, give_dose1_children, give_dose1_adults, give_dose2_adults, s_ij_gen_pop, s_ij_sex, children_dose1_denom, adults_dose1_denom, adults_dose2_denom, lambda_hh, lambda_s, children_dose1_prob, adults_dose1_prob, adults_dose2_prob, lambda, children_dose1_group, adults_dose1_group, adults_dose2_group, p_SE, p_hh, p_s, p_hc, n_vaccination_t_S_children, n_vaccination_t_S_adults, n_vaccination_t_Ea_children, n_vaccination_t_Ea_adults, n_vaccination_t_S, n_vaccination_t_Eb_children, n_vaccination_t_Eb_adults, n_vaccination_t_Ea, delta_S_n_vaccination, n_vaccination_t_R_children, n_vaccination_t_R_adults, n_vaccination_t_Eb, delta_Ea_n_vaccination, n_SEa, n_vaccination_t_R, delta_Eb_n_vaccination, new_S, n_SEa_hh, n_EaEb, delta_R_n_vaccination, n_vaccination_t, n_SEa_s, n_EbI, delta_Ea, new_Ea, new_R, n_SEa_hc, n_EbId, delta_Eb, new_Eb, n_SEa_z, n_EbIr, delta_Id, new_Id, new_E, delta_Ir, new_Ir, new_I, new_N, new_target_met_children_t, new_target_met_adults_t, seed};
  }
  ...
  static void initial(real_type time, const shared_state& shared, internal_state& internal, rng_state_type& rng_state, real_type* state) {
    for (size_t i = 1; i <= shared.dim.seed.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.seed.dim[1]; ++j) {
        internal.seed[i - 1 + (j - 1) * shared.dim.seed.mult[1]] = monty::math::min<real_type>(monty::random::poisson<real_type>(rng_state, shared.seed_rate[i - 1 + (j - 1) * shared.dim.seed.mult[1]]), shared.S0[i - 1 + (j - 1) * shared.dim.S0.mult[1]]);
      }
    }
    state[0] = 1;
    state[1] = 1;
    ...
    for (size_t i = 1; i <= shared.dim.cases_cumulative_by_age.size; ++i) {
      state[i - 1 + shared.odin.offset.state[119]] = 0;
    }
    state[45] = 0;
    state[46] = 0;
    state[47] = 0;
  ...
  static void update(real_type time, real_type dt, const real_type* state, const shared_state& shared, internal_state& internal, rng_state_type& rng_state, real_type* state_next) {
    ...
    const auto * cases_cumulative_by_age = state + shared.odin.offset.state[119];
    ...
    for (size_t i = 1; i <= shared.dim.target_met_children_t.size; ++i) {
      internal.target_met_children_t[i - 1] = (internal.coverage_target_1st_dose_children[i - 1] == 0 ? 0 : ((dust2::array::sum<real_type>(N, shared.dim.N, {i - 1, i - 1}, {2, 3}) * shared.is_child[i - 1]) >= shared.prioritisation_strategy_children[i - 1 + (prioritisation_step_1st_dose_children - 1) * shared.dim.prioritisation_strategy_children.mult[1]] * dust2::array::sum<real_type>(N, shared.dim.N, {i - 1, i - 1}, {1, 3})));
    }
    ...
    for (size_t i = 1; i <= shared.dim.target_met_adults_t.dim[0]; ++i) {
      internal.target_met_adults_t[i - 1 + 2 * shared.dim.target_met_adults_t.mult[1]] = (internal.coverage_target_1st_dose_adults[i - 1] == 0 ? 0 : ((dust2::array::sum<real_type>(N, shared.dim.N, {i - 1, i - 1}, {2, 3}) * (1 - shared.is_child[i - 1])) >= shared.prioritisation_strategy_adults[i - 1 + (prioritisation_step_1st_dose_adults - 1) * shared.dim.prioritisation_strategy_adults.mult[1]] * dust2::array::sum<real_type>(N, shared.dim.N, {i - 1, i - 1}, {1, 3})));
    }
    for (size_t i = 1; i <= shared.dim.target_met_adults_t.dim[0]; ++i) {
      internal.target_met_adults_t[i - 1 + 3 * shared.dim.target_met_adults_t.mult[1]] = (internal.coverage_target_2nd_dose_adults[i - 1] == 0 ? 0 : (((N[i - 1 + 3 * shared.dim.N.mult[1]] + D[i - 1 + 2 * shared.dim.D.mult[1]]) * (1 - shared.is_child[i - 1])) >= shared.prioritisation_strategy_adults[i - 1 + (prioritisation_step_2nd_dose_adults - 1) * shared.dim.prioritisation_strategy_adults.mult[1]] * dust2::array::sum<real_type>(N, shared.dim.N, {i - 1, i - 1}, {1, 3})));
    }
    for (size_t i = 1; i <= shared.dim.D.dim[0]; ++i) {
      for (size_t j = 1; j <= shared.dim.D.dim[1]; ++j) {
    ...
    internal.children_dose1_group[0] = (dust2::array::sum<real_type>(internal.children_dose1_denom.data(), shared.dim.children_dose1_denom) == 0 ? 0 : monty::math::min<real_type>(monty::random::binomial<real_type>(rng_state, internal.daily_doses_children_t[1], internal.children_dose1_prob[0]), internal.max_vax_remaining[0]));
    for (size_t i = 2; i <= static_cast<size_t>(shared.n_group); ++i) {
      internal.children_dose1_group[i - 1] = (dust2::array::sum<real_type>(internal.children_dose1_prob.data(), shared.dim.children_dose1_prob, {i - 1, shared.n_group - 1}) == 0 ? 0 : monty::math::min<real_type>(monty::random::binomial<real_type>(rng_state, internal.daily_doses_children_t[1] - dust2::array::sum<real_type>(internal.children_dose1_group.data(), shared.dim.children_dose1_group, {0, (i - 1) - 1}), internal.children_dose1_prob[i - 1] / dust2::array::sum<real_type>(internal.children_dose1_prob.data(), shared.dim.children_dose1_prob, {i - 1, shared.n_group - 1})), internal.max_vax_remaining[i - 1]));
    }
    internal.adults_dose1_group[0] = (dust2::array::sum<real_type>(internal.adults_dose1_denom.data(), shared.dim.adults_dose1_denom) == 0 ? 0 : monty::math::min<real_type>(monty::random::binomial<real_type>(rng_state, internal.daily_doses_adults_t[1], internal.adults_dose1_prob[0]), internal.max_vax_remaining[0]));
    for (size_t i = 2; i <= static_cast<size_t>(shared.n_group); ++i) {
      internal.adults_dose1_group[i - 1] = (dust2::array::sum<real_type>(internal.adults_dose1_prob.data(), shared.dim.adults_dose1_prob, {i - 1, shared.n_group - 1}) == 0 ? 0 : monty::math::min<real_type>(monty::random::binomial<real_type>(rng_state, internal.daily_doses_adults_t[1] - dust2::array::sum<real_type>(internal.adults_dose1_group.data(), shared.dim.adults_dose1_group, {0, (i - 1) - 1}), internal.adults_dose1_prob[i - 1] / dust2::array::sum<real_type>(internal.adults_dose1_prob.data(), shared.dim.adults_dose1_prob, {i - 1, shared.n_group - 1})), internal.max_vax_remaining[i - 1]));
    }
    ...
    for (size_t i = 1; i <= shared.dim.n_vaccination_t_S_children.size; ++i) {
      internal.n_vaccination_t_S_children[i - 1] = (S[i - 1 + shared.dim.S.mult[1]] + Ea[i - 1 + shared.dim.Ea.mult[1]] + Eb[i - 1 + shared.dim.Eb.mult[1]] + R[i - 1 + shared.dim.R.mult[1]] == 0 ? 0 : monty::math::min<real_type>(monty::random::binomial<real_type>(rng_state, internal.children_dose1_group[i - 1], S[i - 1 + shared.dim.S.mult[1]] / (S[i - 1 + shared.dim.S.mult[1]] + Ea[i - 1 + shared.dim.Ea.mult[1]] + Eb[i - 1 + shared.dim.Eb.mult[1]] + R[i - 1 + shared.dim.R.mult[1]])), S[i - 1 + shared.dim.S.mult[1]]));
    }
    for (size_t i = 1; i <= shared.dim.n_vaccination_t_S_adults.size; ++i) {
      internal.n_vaccination_t_S_adults[i - 1] = (S[i - 1 + shared.dim.S.mult[1]] + Ea[i - 1 + shared.dim.Ea.mult[1]] + Eb[i - 1 + shared.dim.Eb.mult[1]] + R[i - 1 + shared.dim.R.mult[1]] == 0 ? 0 : monty::math::min<real_type>(monty::random::binomial<real_type>(rng_state, internal.adults_dose1_group[i - 1], S[i - 1 + shared.dim.S.mult[1]] / (S[i - 1 + shared.dim.S.mult[1]] + Ea[i - 1 + shared.dim.Ea.mult[1]] + Eb[i - 1 + shared.dim.Eb.mult[1]] + R[i - 1 + shared.dim.R.mult[1]])), S[i - 1 + shared.dim.S.mult[1]]));
    }
    for (size_t i = 1; i <= shared.dim.n_vaccination_t_Ea_children.size; ++i) {
      internal.n_vaccination_t_Ea_children[i - 1] = (Ea[i - 1 + shared.dim.Ea.mult[1]] + Eb[i - 1 + shared.dim.Eb.mult[1]] + R[i - 1 + shared.dim.R.mult[1]] == 0 ? 0 : monty::math::min<real_type>(monty::random::binomial<real_type>(rng_state, internal.children_dose1_group[i - 1] - internal.n_vaccination_t_S_children[i - 1], Ea[i - 1 + shared.dim.Ea.mult[1]] / (Ea[i - 1 + shared.dim.Ea.mult[1]] + Eb[i - 1 + shared.dim.Eb.mult[1]] + R[i - 1 + shared.dim.R.mult[1]])), Ea[i - 1 + shared.dim.Ea.mult[1]]));
    }
    for (size_t i = 1; i <= shared.dim.n_vaccination_t_Ea_adults.size; ++i) {
      internal.n_vaccination_t_Ea_adults[i - 1] = (Ea[i - 1 + shared.dim.Ea.mult[1]] + Eb[i - 1 + shared.dim.Eb.mult[1]] + R[i - 1 + shared.dim.R.mult[1]] == 0 ? 0 : monty::math::min<real_type>(monty::random::binomial<real_type>(rng_state, internal.adults_dose1_group[i - 1] - internal.n_vaccination_t_S_adults[i - 1], Ea[i - 1 + shared.dim.Ea.mult[1]] / (Ea[i - 1 + shared.dim.Ea.mult[1]] + Eb[i - 1 + shared.dim.Eb.mult[1]] + R[i - 1 + shared.dim.R.mult[1]])), Ea[i - 1 + shared.dim.Ea.mult[1]]));
    }
    ...
    for (size_t i = 1; i <= shared.dim.n_vaccination_t_S.dim[0]; ++i) {
      internal.n_vaccination_t_S[i - 1 + 2 * shared.dim.n_vaccination_t_S.mult[1]] = (S[i - 1 + 2 * shared.dim.S.mult[1]] + Ea[i - 1 + 2 * shared.dim.Ea.mult[1]] + Eb[i - 1 + 2 * shared.dim.Eb.mult[1]] + R[i - 1 + 2 * shared.dim.R.mult[1]] == 0 ? 0 : monty::math::min<real_type>(monty::random::binomial<real_type>(rng_state, internal.adults_dose2_group[i - 1], S[i - 1 + 2 * shared.dim.S.mult[1]] / (S[i - 1 + 2 * shared.dim.S.mult[1]] + Ea[i - 1 + 2 * shared.dim.Ea.mult[1]] + Eb[i - 1 + 2 * shared.dim.Eb.mult[1]] + R[i - 1 + 2 * shared.dim.R.mult[1]])), S[i - 1 + 2 * shared.dim.S.mult[1]]));
    }
    for (size_t i = 1; i <= shared.dim.n_vaccination_t_Eb_children.size; ++i) {
      internal.n_vaccination_t_Eb_children[i - 1] = (Eb[i - 1 + shared.dim.Eb.mult[1]] + R[i - 1 + shared.dim.R.mult[1]] == 0 ? 0 : monty::math::min<real_type>(monty::random::binomial<real_type>(rng_state, internal.children_dose1_group[i - 1] - internal.n_vaccination_t_S_children[i - 1] - internal.n_vaccination_t_Ea_children[i - 1], Eb[i - 1 + shared.dim.Eb.mult[1]] / (Eb[i - 1 + shared.dim.Eb.mult[1]] + R[i - 1 + shared.dim.R.mult[1]])), Eb[i - 1 + shared.dim.Eb.mult[1]]));
    }
    for (size_t i = 1; i <= shared.dim.n_vaccination_t_Eb_adults.size; ++i) {
      internal.n_vaccination_t_Eb_adults[i - 1] = (Eb[i - 1 + shared.dim.Eb.mult[1]] + R[i - 1 + shared.dim.R.mult[1]] == 0 ? 0 : monty::math::min<real_type>(monty::random::binomial<real_type>(rng_state, internal.adults_dose1_group[i - 1] - internal.n_vaccination_t_S_adults[i - 1] - internal.n_vaccination_t_Ea_adults[i - 1], Eb[i - 1 + shared.dim.Eb.mult[1]] / (Eb[i - 1 + shared.dim.Eb.mult[1]] + R[i - 1 + shared.dim.R.mult[1]])), Eb[i - 1 + shared.dim.Eb.mult[1]]));
    }
    ...
    for (size_t i = 1; i <= shared.dim.n_vaccination_t_Ea.dim[0]; ++i) {
      internal.n_vaccination_t_Ea[i - 1 + 2 * shared.dim.n_vaccination_t_Ea.mult[1]] = (Ea[i - 1 + 2 * shared.dim.Ea.mult[1]] + Eb[i - 1 + 2 * shared.dim.Eb.mult[1]] + R[i - 1 + 2 * shared.dim.R.mult[1]] == 0 ? 0 : monty::math::min<real_type>(monty::random::binomial<real_type>(rng_state, internal.adults_dose2_group[i - 1] - internal.n_vaccination_t_S[i - 1 + 2 * shared.dim.n_vaccination_t_S.mult[1]], Ea[i - 1 + 2 * shared.dim.Ea.mult[1]] / (Ea[i - 1 + 2 * shared.dim.Ea.mult[1]] + Eb[i - 1 + 2 * shared.dim.Eb.mult[1]] + R[i - 1 + 2 * shared.dim.R.mult[1]])), Ea[i - 1 + 2 * shared.dim.Ea.mult[1]]));
    }
    ...
    for (size_t i = 1; i <= shared.dim.n_vaccination_t_R_children.size; ++i) {
      internal.n_vaccination_t_R_children[i - 1] = (R[i - 1 + shared.dim.R.mult[1]] == 0 ? 0 : monty::math::min<real_type>(internal.children_dose1_group[i - 1] - internal.n_vaccination_t_S_children[i - 1] - internal.n_vaccination_t_Ea_children[i - 1] - internal.n_vaccination_t_Eb_children[i - 1], R[i - 1 + shared.dim.R.mult[1]]));
    }
    for (size_t i = 1; i <= shared.dim.n_vaccination_t_R_adults.size; ++i) {
      internal.n_vaccination_t_R_adults[i - 1] = (R[i - 1 + shared.dim.R.mult[1]] == 0 ? 0 : monty::math::min<real_type>(internal.adults_dose1_group[i - 1] - internal.n_vaccination_t_S_adults[i - 1] - internal.n_vaccination_t_Ea_adults[i - 1] - internal.n_vaccination_t_Eb_adults[i - 1], R[i - 1 + shared.dim.R.mult[1]]));
    }
    ...
    for (size_t i = 1; i <= shared.dim.new_target_met_children_t.size; ++i) {
      internal.new_target_met_children_t[i - 1] = (internal.coverage_target_1st_dose_children[i - 1] == 0 ? 0 : ((dust2::array::sum<real_type>(internal.new_N.data(), shared.dim.N, {i - 1, i - 1}, {2, 3}) * shared.is_child[i - 1]) >= shared.prioritisation_strategy_children[i - 1 + (prioritisation_step_1st_dose_children - 1) * shared.dim.prioritisation_strategy_children.mult[1]] * dust2::array::sum<real_type>(internal.new_N.data(), shared.dim.N, {i - 1, i - 1}, {1, 3})));
    }
    for (size_t i = 1; i <= shared.dim.new_target_met_adults_t.dim[0]; ++i) {
      for (size_t j = 1; j <= 2; ++j) {
        internal.new_target_met_adults_t[i - 1 + (j - 1) * shared.dim.new_target_met_adults_t.mult[1]] = 0;
      }
    }
    for (size_t i = 1; i <= shared.dim.new_target_met_adults_t.dim[0]; ++i) {
      internal.new_target_met_adults_t[i - 1 + 2 * shared.dim.new_target_met_adults_t.mult[1]] = (internal.coverage_target_1st_dose_adults[i - 1] == 0 ? 0 : ((dust2::array::sum<real_type>(internal.new_N.data(), shared.dim.N, {i - 1, i - 1}, {2, 3}) * (1 - shared.is_child[i - 1])) >= shared.prioritisation_strategy_adults[i - 1 + (prioritisation_step_1st_dose_adults - 1) * shared.dim.prioritisation_strategy_adults.mult[1]] * dust2::array::sum<real_type>(internal.new_N.data(), shared.dim.N, {i - 1, i - 1}, {1, 3})));
    }
    for (size_t i = 1; i <= shared.dim.new_target_met_adults_t.dim[0]; ++i) {
      internal.new_target_met_adults_t[i - 1 + 3 * shared.dim.new_target_met_adults_t.mult[1]] = (internal.coverage_target_2nd_dose_adults[i - 1] == 0 ? 0 : (((internal.new_N[i - 1 + 3 * shared.dim.N.mult[1]] + internal.new_D[i - 1 + 2 * shared.dim.D.mult[1]]) * (1 - shared.is_child[i - 1])) >= shared.prioritisation_strategy_adults[i - 1 + (prioritisation_step_2nd_dose_adults - 1) * shared.dim.prioritisation_strategy_adults.mult[1]] * dust2::array::sum<real_type>(internal.new_N.data(), shared.dim.N, {i - 1, i - 1}, {1, 3})));
    }
    const real_type prioritisation_step_1st_dose_children_proposal = (dust2::array::sum<real_type>(internal.new_target_met_children_t.data(), shared.dim.new_target_met_children_t) == dust2::array::sum<real_type>(internal.coverage_target_1st_dose_children.data(), shared.dim.coverage_target_1st_dose_children) ? prioritisation_step_1st_dose_children + 1 : prioritisation_step_1st_dose_children);
    const real_type prioritisation_step_1st_dose_adults_proposal = (dust2::array::sum<real_type>(internal.new_target_met_adults_t.data(), shared.dim.new_target_met_adults_t, {0, shared.dim.new_target_met_adults_t.dim[0] - 1}, {2, 2}) == dust2::array::sum<real_type>(internal.coverage_target_1st_dose_adults.data(), shared.dim.coverage_target_1st_dose_adults) ? prioritisation_step_1st_dose_adults + 1 : prioritisation_step_1st_dose_adults);
    const real_type prioritisation_step_2nd_dose_adults_proposal = (dust2::array::sum<real_type>(internal.new_target_met_adults_t.data(), shared.dim.new_target_met_adults_t, {0, shared.dim.new_target_met_adults_t.dim[0] - 1}, {3, 3}) == dust2::array::sum<real_type>(internal.coverage_target_2nd_dose_adults.data(), shared.dim.coverage_target_2nd_dose_adults) ? prioritisation_step_2nd_dose_adults + 1 : prioritisation_step_2nd_dose_adults);
    ...
    for (size_t i = 1; i <= shared.dim.cases_cumulative_by_age.size; ++i) {
      state_next[i - 1 + shared.odin.offset.state[119]] = cases_cumulative_by_age[i - 1] + dust2::array::sum<real_type>(internal.n_SEa.data(), shared.dim.n_SEa, {i - 1, i - 1}, {0, shared.dim.n_SEa.dim[1] - 1});
    }
  ...
  static real_type compare_data(real_type time, const real_type* state, const data_type& data, const shared_state& shared, internal_state& internal, rng_state_type& rng_state) {
    ...
    odin_ll += unless_nan(monty::density::negative_binomial_mu(data.cases, 1 / shared.alpha_cases, model_cases, true));
    odin_ll += unless_nan(monty::density::negative_binomial_mu(data.cases_00_04, 1 / shared.alpha_cases_00_04, model_cases_00_04, true));
    odin_ll += unless_nan(monty::density::negative_binomial_mu(data.cases_05_14, 1 / shared.alpha_cases_05_14, model_cases_05_14, true));
    odin_ll += unless_nan(monty::density::negative_binomial_mu(data.cases_15_plus, 1 / shared.alpha_cases_15_plus, model_cases_15_plus, true));
    odin_ll += unless_nan(monty::density::negative_binomial_mu(data.deaths, 1 / shared.alpha_deaths, model_deaths, true));
    odin_ll += unless_nan(monty::density::negative_binomial_mu(data.deaths_00_04, 1 / shared.alpha_deaths_00_04, model_deaths_00_04, true));
    odin_ll += unless_nan(monty::density::negative_binomial_mu(data.deaths_05_14, 1 / shared.alpha_deaths_05_14, model_deaths_05_14, true));
    odin_ll += unless_nan(monty::density::negative_binomial_mu(data.deaths_15_plus, 1 / shared.alpha_deaths_15_plus, model_deaths_15_plus, true));
    odin_ll += unless_nan(monty::density::beta(data.cfr_00_04, deaths_cumulative_00_04, cases_cumulative_00_04 - deaths_cumulative_00_04, true));
    odin_ll += unless_nan(monty::density::beta(data.cfr_05_14, deaths_cumulative_05_14, cases_cumulative_05_14 - deaths_cumulative_05_14, true));
    odin_ll += unless_nan(monty::density::beta(data.cfr_15_plus, deaths_cumulative_15_plus, cases_cumulative_15_plus - deaths_cumulative_15_plus, true));
    odin_ll += unless_nan(monty::density::binomial(data.cases_HCW, data.cases_total, model_prop_HCW, true));
    odin_ll += unless_nan(monty::density::binomial(data.cases_SW, data.cases_total, model_prop_SW, true));
    odin_ll += unless_nan(monty::density::beta_binomial_prob(data.cases_00_04_binom, data.cases_00_14_binom, model_cases_00_04 / model_cases_00_14, shared.rho_00_04, true));
    odin_ll += unless_nan(monty::density::beta_binomial_prob(data.cases_00_14_binom, data.cases_binom, model_cases_00_14 / model_cases, shared.rho_00_14, true));
    return odin_ll;
  }
};
...
SEXP dust2_unfilter_model_targeted_vax_run(cpp11::sexp ptr, cpp11::sexp r_initial, bool save_trajectories, cpp11::sexp save_snapshots, bool adjoint, cpp11::sexp r_index_state, cpp11::sexp r_index_group, bool preserve_particle_dimension, bool preserve_group_dimension) {
  return dust2::r::dust2_unfilter_run<dust2::dust_discrete<model_targeted_vax>>(ptr, r_initial, save_trajectories, save_snapshots, adjoint, r_index_state, r_index_group, preserve_particle_dimension, preserve_group_dimension);
}
...
SEXP dust2_unfilter_model_targeted_vax_last_snapshots(cpp11::sexp ptr, bool select_random_particle, bool preserve_particle_dimension, bool preserve_group_dimension) {
  return dust2::r::dust2_unfilter_last_snapshots<dust2::dust_discrete<model_targeted_vax>>(ptr, select_random_particle, preserve_particle_dimension, preserve_group_dimension);
}
...
SEXP dust2_filter_model_targeted_vax_run(cpp11::sexp ptr, cpp11::sexp r_initial, bool save_trajectories, cpp11::sexp save_snapshots, bool adjoint, cpp11::sexp index_state, cpp11::sexp index_group, bool preserve_particle_dimension, bool preserve_group_dimension) {
  return dust2::r::dust2_filter_run<dust2::dust_discrete<model_targeted_vax>>(ptr, r_initial, save_trajectories, save_snapshots, adjoint, index_state, index_group, preserve_particle_dimension, preserve_group_dimension);
}
...
SEXP dust2_filter_model_targeted_vax_last_snapshots(cpp11::sexp ptr, bool select_random_particle, bool preserve_particle_dimension, bool preserve_group_dimension) {
  return dust2::r::dust2_filter_last_snapshots<dust2::dust_discrete<model_targeted_vax>>(ptr, select_random_particle, preserve_particle_dimension, preserve_group_dimension);
}
